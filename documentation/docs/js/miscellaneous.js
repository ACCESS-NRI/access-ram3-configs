...
*/
// The window.siteUrl variable is set in overrides/main.html
function makeLinksExternal() {
    // Open external links in a new tab
    const externalLinks = [...document.querySelectorAll('a[href^="http"]')]
        .filter(link => new URL(link.href).hostname !== new URL(window.siteUrl).hostname);
    ...
    // Add external link icon only to:
    const externalIconLinks = [...externalLinks].filter(link => {
        return link.closest('article') && // links within the main article 
            !link.matches('.vertical-card, .horizontal-card, .text-card') && // not links on cards
            !link.matches('.md-content__button.md-icon[title="Edit this page"]') // not edit in github icon
    }).forEach(link => {link.classList.add('external-link')});
}
...
function toggleTerminalAnimations() {
    if (document.querySelector('terminal-window')) {
        const COOKIE_TEXT = `${window.siteUrl}-animated-terminal-state`;
